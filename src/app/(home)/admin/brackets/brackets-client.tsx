...
export function BracketsClient({
  ...
  const takenSeasonIds = new Set(brackets.map((bracket) => bracket.seasonId))
  const defaultSeasonId = () => {
    const active = seasons.find(
      (season) => season.isActive && !takenSeasonIds.has(season.id)
    )
    return active ? String(active.id) : 'none'
  }
  ...
  function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (seasonId === 'none') {
      setError('Pick the season these playoffs belong to')
      return
    }
    setError(null)
    ...
    <div className='flex flex-col gap-4'>
      ...
      <Dialog open={isCreateOpen} onOpenChange={handleDialogChange}>
        <DialogContent className='max-h-[85vh] overflow-y-auto'>
          <DialogHeader>
            <DialogTitle>New Playoff Bracket</DialogTitle>
            <DialogDescription>
              One single-elimination bracket per season, named after it
              (&quot;Season 7 Playoffs&quot;). You&apos;ll add players and set
              matchups on the next screen.
            </DialogDescription>
          </DialogHeader>
                  ...
                  <SelectContent>
                    ...
                    {seasons.map((season) => (
                      <SelectItem
                        ...
                        disabled={takenSeasonIds.has(season.id)}
                      >
                        {season.name}
                        {takenSeasonIds.has(season.id) ? ' (has bracket)' : ''}
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
